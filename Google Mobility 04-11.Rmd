...
```{r}
google.transform1 <- powerTransform(cbind(cacovid_mobility$retail_and_recreation_percent_change_from_baseline, cacovid_mobility$grocery_and_pharmacy_percent_change_from_baseline) ~ 1)
...
```{r}
bc.transform <- boxCox(google.category2)
...
plot(hatvalues(google.category3), rstandard(google.category3), xlab = "Leverage", 
     ylab = "Standardized Residuals", lwd = 0.8, cex = 0.8)
abline(v = 4*(p+1)/n, col = "red", lty = 2)
abline(h = c(-2,2), col = "blue", lty =2)
```

Checking for high leverage points and outliers
```{r}
# leverage point calculations
p <- 7
n <- nrow(cacovid_mobility)
google.hat <- hatvalues(google.category3)
which(google.hat > 4*(p+1)/n)
```
