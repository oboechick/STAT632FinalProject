---
title: "Correlations to COVID-19 Deaths in California"
...
**Variables used (for more details see Appendix 5):**  
...
pairs(deaths ~ confirmed + tests + stringency_index + retail_and_recreation_percent_change_from_baseline + 
...
#### 2. 
Are the trends in movement significantly correlated to deaths caused by COVID-19? We plan to use *deaths* as the response; *confirmed*, *tests*, *contact tracing*, and *stringency index* from in the original COVID-19 data set; and *retail and recreation percent change from baseline*, *grocery and pharmacy percent change from baseline*, *parks percent change from baseline*, *transit stations percent change from baseline*, *workplaces percent change from baseline*, *residential percent change from baseline* from the Google Mobility data set to answer this question.
...
In our last analysis, we wanted to see if we could find a linear regression of the Google Mobility variables when deaths as a response. So, we started off by doing an initial summary, and our initial plots also showed that linearity assumptions were not satisfied. It also showed that Workplace change was not significant, which was interesting since we assumed that most workplaces were closed. 
Because the current model did not meet our assumptions of linearity, we decided to run a variable selection to help us narrow down significant predictors. We compared all models by looking at adjusted R-squared and CP values. (Table 2, Appendix 2B) The model we chose had all 6 predictor variables. Next, we decided to check if there were any necessary transformations for the predictors. We found that 5 out of 6 variables needed to be transformed.(Table 3, Appendix 2B).The assumptions were still not met (Plot 2, Appendix 2B). Then we needed to see if the Response variable needed to be transformed, and found that it required a square root transformation (Plot 3, Appendix 2B).
Prior to transformation, when we did model selection using reg.susbset, we found that the CP and BIC indicated the 5 predictor model was better, but when we looked at the difference in Adjusted R^2 between the 6 vs. 5 predictor model, we found that they were very similar, at 34.328642 vs. 34.328653. 
...
Next we did a variable selection using AIC and BIC stepwise selection (see Code 1-2, Appendix 1). We can see that the only variable that is not significant is a dummy variable for *testing policy* and we cannot remove it so we leave it in.

After doing some analysis (Plot 5-6, Code 3, Appendix 1) we determined that we needed to do a transformation and this is the final model that we ended with. $\sqrt[3]{deaths} = \beta_0 + \beta_1 fstay.home.restrictions + \beta_2 \sqrt[3]{tests}  + \beta_3 fworkplace.closing  + \beta_4 fschool.closing  + \beta_5 fcontact.tracing  + \beta_6ftesting.policy  + \beta_7 log(stringency.index)$
...
Manipulating the Google mobility data into a workable data set resulted in the following predictors: *retail_and_recreation_percent_change_from_baseline*, *grocery_and_pharmacy_percent_change_from_baseline*, *parks_percent_change_from_baseline*,  *transit_stations_percent_change_from_baseline*, *workplaces_percent_change_from_baseline*, and *residential_percent_change_from_baseline*. We then combined them with the full model from the base data set to create our new full model.
...
google.only4 <- lm(sqrt(deaths) ~ (I(retail_and_recreation_percent_change_from_baseline)^2) + 
    ...
    (I(1/residential_percent_change_from_baseline)^2), data = cacovid_mobilityGO)
...
The top two plots are the model using categorical variables and the bottom two plots are the numerical data only. There is a stark difference between the two. The second model is definitely satisfies the linearity assumptions.

### Interpretation:

$\sqrt{deaths} = 0.2876+347.5425retail\;and\;recreation\;percent\;change\;from\;baseline^2-536.4473\sqrt{grocery\;and\;pharmacy\;percent\;change\;from\;baseline}-15.9176log(parks\;percent\;change\;from\;baseline)-44.9572log(transit\;stations\;percent\;change\;from\;baseline)-55.1082 workplaces\;percent\;change\;from\;baseline+392.1944residential\;percent\;change\;from\;baseline)^{-2}$

We come to the conclusion that this model explains 43.11% of the variability seen in the number of California Covid deaths, a one unit increase in people going to areas of $retail\;and\;recreation^2$ with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by 347.54, a one unit increase in people going to areas of $\sqrt{grocery\;and\;pharmacy}$ with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by -536.45, a one unit increase in people going to areas of $log(parks)$ with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by -15.92, a one unit increase in people going to areas of $log(transit\;stations)$ with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by -44.96, a one unit increase in people going to areas of workplaces with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by -55.11, and a one unit increase in people going to areas of $residential^{-2}$ with the other predictors held fixed is associated with an increase in $\sqrt{deaths}$ by 392.19. 

## Conclusions

For the first question, a linear regression model using only variables from the base data set did not meet the required assumptions of normality and thus we were unable to answer this question..

For the second question, a linear regression model using variables from base data and Google mobility data did not meet the required assumptions of normality when we included any categorical variables as predictors. So we created a linear regression model using only numerical variables from the Google mobility data, this came the closest to meeting assumptions of normality. From this we found that the Retail and Recreation variable as well as the Residential Variable are positively correlated with deaths and the Grocery and Pharmacy, Parks, Workplaces and Transit Station are negatively correlated with deaths.

For the third question, we were unable to find a usable data set and thus were unable to answer this question.
