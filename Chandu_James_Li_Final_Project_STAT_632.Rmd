...
Our assumption that confirmed would have a strong effect on deaths was not correct, however, no matter what we did with the data, it is not a linear model since there is a clear pattern in the residuals vs fitted plot. So while we could interpret this model, it will not actually mean what we want because it is not linear.
...
For our next model, we decided to remove all base COVID-19 numerical data from the original data set. The model included categorical base COVID-19 data and Google mobility data. After exploring this model, we came to the same conclusion as the previous model, that assumptions of linearity were not met. Because of these results, we decided to remove all base COVID-19 data and work solely with Google mobility data. 
